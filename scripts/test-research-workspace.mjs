...

{
  const r = spawnSync(
    process.execPath,
    ["--experimental-strip-types", "scripts/test-research-terrain.ts"],
    { cwd: root, encoding: "utf8" },
  );
  assert.equal(r.status, 0, `research-terrain\n${r.stdout}\n${r.stderr}`);
}

{
  const r = spawnSync(
    process.execPath,
    ["--experimental-strip-types", "scripts/test-research-map-engine.ts"],
    { cwd: root, encoding: "utf8" },
  );
...

{
  const r = spawnSync(
    process.execPath,
    ["--experimental-strip-types", "scripts/test-research-imagery.ts"],
    { cwd: root, encoding: "utf8" },
  );
  assert.equal(r.status, 0, `research-imagery\n${r.stdout}\n${r.stderr}`);
}

{
  const r = spawnSync(
    process.execPath,
    ["--experimental-strip-types", "scripts/test-research-world-lab.ts"],
    { cwd: root, encoding: "utf8" },
  );
  assert.equal(r.status, 0, `research-world-lab\n${r.stdout}\n${r.stderr}`);
}
