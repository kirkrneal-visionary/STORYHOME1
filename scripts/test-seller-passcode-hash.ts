...
assert.match(mig, /hmac\(convert_to\(v_norm, 'UTF8'\)/);
assert.match(mig, /grant all on public.seller_access_attempts to service_role/);
const repair = read("supabase/migrations/0044_seller_passcode_hmac_cast.sql");
assert.match(repair, /hmac\(convert_to\(v_norm, 'UTF8'\)/);
assert.match(repair, /Does NOT delete users, listings, or county\/CAD/);
assert.doesNotMatch(repair, /delete from public\.(profiles|listings|county_parcels)/i);
const bytea = read("supabase/migrations/0045_seller_passcode_hmac_bytea.sql");
assert.match(bytea, /hmac\(convert_to\(v_norm, 'UTF8'\)/);
assert.match(bytea, /Does NOT delete users, listings, or county\/CAD/);
assert.doesNotMatch(bytea, /delete from public\.(profiles|listings|county_parcels)/i);
const pathFix = read("supabase/migrations/0046_seller_passcode_extensions_path.sql");
assert.match(pathFix, /search_path = public, extensions/);
assert.match(pathFix, /Does NOT delete users, listings, or county\/CAD/);
assert.doesNotMatch(pathFix, /delete from public\.(profiles|listings|county_parcels)/i);
