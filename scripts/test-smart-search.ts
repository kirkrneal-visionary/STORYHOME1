...
assert.match(hero, /story-home-hero-safe/);
...
assert.match(ghost, /GHOST_RENT_HINT/);
assert.match(ghost, /mode === "rent"/);
...
assert.match(GHOST_RENT_HINT.full, /Rentals are not listed yet/);
...
assert.match(hub, />\s*Clear\s*</);
assert.match(hub, />\s*Apply\s*</);
...
assert.match(hub, /data-mobile-advanced/);
assert.match(hub, /data-region="property"/);
assert.match(hub, /data-region="features"/);
assert.match(hub, /data-region="home"/);
assert.match(hub, /data-region="type"/);
assert.match(hub, /data-region="hoa"/);
assert.match(hub, /data-preview-future/);
assert.match(hub, /Water View/);
assert.match(hub, /Waterfront/);
assert.match(hub, /Golf Course/);
assert.match(hub, /MOBILE_FEATURE_TILES/);
assert.match(hub, /md:hidden/);
assert.match(hub, /hidden md:block/);
assert.match(hub, /sanitizeBoundFields/);
assert.match(hub, /data-filter-count/);
assert.doesNotMatch(hub, /Price & Features/);
assert.doesNotMatch(hub, /Home & Land/);
assert.doesNotMatch(hub, /HomeBoundPickers/);
assert.doesNotMatch(hub, /const FEATURE_TILES/);
assert.doesNotMatch(hub, /Briefcase/);
...
assert.match(hub, /story-home-mobile-advanced/);
assert.match(hub, /story-home-mobile-features/);
assert.match(hub, /data-range-summary/);
assert.match(hub, /MobileRangeEditor/);
assert.match(hub, /countActiveFilters/);
...
assert.throws(() => read("src/components/home/HomeBoundPickers.tsx"));

assert.match(css, /story-home-search-shell/);
assert.match(css, /240ms/);
assert.match(css, /story-home-hero-safe/);
assert.match(css, /--story-header-h-full/);
assert.match(css, /story-home-hero-stage/);
assert.match(css, /header-h-full/);
assert.match(css, /env-1\) 84%/);
...
const heroSrc = read("src/components/home/HomeSearchHero.tsx");
assert.match(heroSrc, /story-home-hero/);
assert.match(heroSrc, /story-home-hero-safe/);
assert.doesNotMatch(heroSrc, /sticky/);
...
assert.doesNotMatch(css, /story-home-price-features/);
assert.doesNotMatch(css, /story-home-home-land/);
assert.doesNotMatch(css, /story-home-range-pair/);
assert.doesNotMatch(css, /story-home-bound-title/);
assert.doesNotMatch(css, /story-home-advanced-body/);
assert.doesNotMatch(css, /story-home-feature-cluster/);
...
assert.match(css, /42rem/);
assert.match(css, /story-home-mobile-editor/);
...
assert.match(css, /--m1-measure-label/);
assert.match(css, /--m1-home-label/);
assert.match(css, /--m1-tile-h/);
assert.match(css, /--m1-icon/);
...
assert.match(heroSrc, /Find your next place in/);
...
assert.deepEqual(boundsFromPreset(buyPreset), { min: "350000", max: "500000" });
assert.deepEqual(boundsFromPreset(rentPreset), { min: "1500", max: "2000" });
assert.equal(matchPreset(BUY_PRICE_PRESETS, "350000", "500000"), "350-500");
assert.equal(matchPreset(RENT_PRICE_PRESETS, "350000", "500000"), "custom");
...
assert.doesNotMatch(filterSrc, /monthlyRent|transactionMode/);
// RENTAL FILTERING NOT IMPLEMENTED — no rental classifier exists.
// Sale fixtures cannot prove Buy/Rent isolation until a follow-up adds
// a listing transaction field plus a monthly-rent value that is not CAD.
