...
| View as buyer / role labels cannot grant neighbor RPCs | `decideStoryProRpc` + `0056` source contract **ran**. Live SQL apply is a later paste |
| Archie/Corridors scoring is not executed in client panels | `test-harden-wave-3-brain` **ran**. Same engines on `/api/shi` |
| Browser no longer dumps `county_parcels` via PostgREST | `test-harden-wave-4-warehouse` **ran**. `0057` is repo-only until pasted |
| Production login bundle has no DevPass; `.map` 404s | `test-harden-wave-5-hygiene` **ran** |
