...
export const SHI_WAVES: ShiWave[] = [
  ...
  },
  {
    ...
    goal: "Agent World under StoryHome OS; Living Mark presence + play respect + agent analytics; Story Walk export later.",
    status: "current",
    frontend: [
      "SW-1: AgentWorldView on /agents/[id]",
      "SW-2: Settings Living Mark library",
      "SW-3: LivingMarkPresence — autoplay, freeze, agent nudge",
      "SW-4: Play respect — guest 4/session · account 4/lifetime · own always",
      "SW-5: Agent World analytics slice — plays/completes/drop-off, guest sessions, account uniques, CTA taps",
      "StoryHome chrome insets (safe-top + bottom clearance)",
    ],
    backend: [
      "Armor scripts/test-story-walk-sw1…sw5.mjs",
      "Migration 0032_living_marks.sql",
      "Migration 0033_agent_world_engagement.sql (agent-owned engagement + RLS)",
    ],
    outOfScope: [
      "SW-6 share profile link (next)",
      "Story Walk compositor (SW-7)",
      "Permanent guest fingerprinting",
      "Opening product_analytics_events to agents",
