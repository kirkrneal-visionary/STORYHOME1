...
export async function getObservationReadiness(
  ...
  // Table exists, tracking started, but no events yet — need a second pull (or fields unchanged).
  return {
    source: src,
    status: "awaiting_next_pull",
    statusLabel: "Awaiting next CAD pull",
    detail:
      "Observation tracking is on, but Archie has not recorded a field change between pulls yet. Empty feed does not mean “nothing changed in the world.”",
    eventsTableAvailable,
    absentColumnAvailable,
    trackingStarted,
    eventCount,
